...
const sessionIdSchema = z.string().trim().min(2).max(100).optional().default('main')
...
export const createMessagingRouter = (service, cfg) => {
  ...
  router.post('/send-image', upload.single('file'), validate(imageSchema), asyncHandler(async (req, res) => {
    const { to, url, caption, fileName, sessionId } = req.body
  ...
  router.post('/send-audio', upload.single('file'), validate(audioSchema), asyncHandler(async (req, res) => {
    const { to, url, fileName, ptt, sessionId } = req.body
  ...
  router.get('/messages', asyncHandler(async (req, res) => {
    ...
    ok(res, {
      counts: service.messages.counts(apiKeyId, sessionId),
      messages: service.messages.list({ status, apiKeyId, sessionId, limit })
    })
  }))

  router.post('/send-location', validate(locationSchema), asyncHandler(async (req, res) => {
    const { to, latitude, longitude, name, address, sessionId } = req.body
